...
.. code-block:: text
   ...
   diff_drive_controller ── /wheel/odom: vx, vy=0 ──┐
                                                    ├──> ekf_filter_node
   Gazebo IMU ───────────── /imu/data: vyaw ─────────┘         ├── /odometry/filtered
                                                              └── TF odom -> base_footprint

``/wheel/odom`` — сырое измерение, а ``/odometry/filtered`` — стабильный
публичный выход локального фильтра. ``world_frame=odom`` запрещает EKF
создавать глобальные скачки: коррекции карты остаются ответственностью
SLAM или AMCL.

Wheel odometry задаёт продольную скорость и нулевую боковую скорость.
Независимая ``/imu/data.angular_velocity.z`` задаёт скорость поворота. EKF не
калибрует радиус и колею автоматически: систематические ошибки геометрии всё
равно исправляются в ``diff_drive_controller``.
...
.. code-block:: bash
   ...
   ros2 launch rtk2026_bringup sim_slam_launch.py use_slam:=false
