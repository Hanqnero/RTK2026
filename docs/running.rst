...
.. list-table:: Точки входа проекта
   ...
   * - ``ros2 launch rtk2026_bringup sim_slam_launch.py use_slam:=false``
     - Gazebo, привод, сенсоры, EKF и RViz без ``slam_toolbox``
   ...
   * - ``ros2 launch rtk2026_observability diagnostics.launch.py use_gui:=true``
     - Диагностика mapping, RQt и PlotJuggler на втором desktop
     - Запускать рядом с sim SLAM.
...
Launch занимает текущий терминал и пишет туда логи. Для teleop, ``rqt_graph``
...
Для симуляционного картирования откройте третий CONTAINER-терминал:
...
проверяют ``/imu/data``, joints и ground truth. На реальном роботе до
