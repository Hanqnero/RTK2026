#!/usr/bin/env python3
"""Стендовая последовательность команд для проверки motor firmware.
...
class Command:
    """Именованная команда скорости, записываемая как отдельная фаза лога."""
...
def parse_args() -> argparse.Namespace:
    """Разобрать порт, длительность фаз и путь CSV."""
...
def encode_command(linear_mps: float, angular_rps: float, debug_raw_encoder: int) -> bytes:
    """Упаковать один 9-байтовый ``ControlPacket``."""
...
def write_log_row(writer: csv.writer, row_type: str, **fields: object) -> None:
    """Записать command или telemetry row с заданным порядком keyword-полей."""

    writer.writerow([row_type] + [fields[k] for k in fields])


def main() -> int:
    """Выполнить последовательность направлений и сохранить команды/ответы."""
