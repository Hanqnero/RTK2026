#!/usr/bin/env python3
"""Проверка согласованности команд колёс, PWM и знаков энкодеров."""
...
def parse_args() -> argparse.Namespace:
    """Разобрать безопасную скорость, длительность импульса и паузы."""
...
def command_for_wheels(left_mps: float, right_mps: float) -> bytes:
    """Преобразовать независимые скорости колёс в body linear/angular."""
...
def write_wheels(ser: serial.Serial, left_mps: float, right_mps: float) -> None:
    """Передать команду колёс и принудительно вытолкнуть буфер Serial."""
...
def read_packets_until(
    ...
    """Собрать все полные пакеты, полученные до monotonic deadline."""
...
def send_stop(ser: serial.Serial, seconds: float) -> None:
    """Повторять нулевую команду заданное время и дренировать ответы."""
...
def sign(value: int) -> int:
    """Вернуть -1, 0 или +1 без преобразования к float."""
...
def run_case(
    ...
    """Проверить один борт в одном направлении и вывести PASS/FAIL."""
...
def main() -> int:
    """Выполнить четыре проверки знака, гарантируя остановку в finally."""
