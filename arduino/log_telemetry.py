#!/usr/bin/env python3
"""Запись и отображение бинарной телеметрии Arduino.
...
class TelemetrySample:
    """Одна декодированная выборка с временной меткой хоста."""
    ...
    def odom_heading_deg(self) -> float:
        """Курс одометрии в градусах для человекочитаемого вывода."""

        return self.odom_heading_rad * 180.0 / math.pi


def parse_args() -> argparse.Namespace:
    """Разобрать параметры Serial, частоты вывода и CSV-файла."""
...
def format_sample(sample: TelemetrySample) -> str:
    """Сформировать одну компактную человекочитаемую строку телеметрии."""
...
def write_csv_row(writer: csv.writer, sample: TelemetrySample) -> None:
    """Записать выборку в CSV в стабильном порядке полей."""
...
def main() -> int:
    """Открыть Serial и читать полные 32-байтовые пакеты до прерывания."""
