...
которым робот подъехал. Хорда между её концами здесь не годится: цепочка
может изгибаться, и прямая от её начала к концу курса робота не даёт.

Направление отбытия, наоборот, берётся хордой от текущей вершины к целевой,
без промежуточных. Первый сегмент исходящей цепочки не годится: несколько
цепочек могут уходить из вершины по одному ребру и разойтись дальше, в
геометрической вершине. Первый сегмент у них тогда общий, и различить их
нечем, хотя ведут они в разные стороны. Хорда различает.
"""
...
def chord_direction(
    ...
    """Направление прямой между двумя вершинами — направление отбытия.

    ``None``, если вершины совпадают: направления у такой пары нет.
    """
