...
def _check_determinism(table: ManeuverTable) -> list[Finding]:
    ...
    Нарушается, когда два выхода попали в один класс и разделение
    по порядку угла их не развело: такое бывает при более трёх выходах
    в одном классе.
    """
    findings: list[Finding] = []
                ...
                findings.append(
                    Finding(
                        ...
                        message=(
                            ...
                            "знак не сможет выбрать однозначно"
                        ),
                        state=state,
                    )
                )
