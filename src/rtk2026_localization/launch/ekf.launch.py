...
Источник локальной одометрии задаётся выбранным YAML. Штатные конфигурации
симуляции и реального робота используют единый интерфейс ``/wheel/odom``;
``/ground_truth/odom`` остаётся только эталоном эксперимента. Нода публикует
``/odometry/filtered`` и владеет динамической трансформацией
``odom -> base_footprint``. Глобальная трансформация ``map -> odom`` в этот
launch не входит.
"""
