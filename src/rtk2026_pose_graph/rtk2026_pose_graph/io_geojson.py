...
Всё остальное из ``properties`` попадает в ``metadata`` как есть. Список
прикладных ключей здесь не перечисляется намеренно: добавление нового
правила движения не должно требовать правки загрузчика.
"""
