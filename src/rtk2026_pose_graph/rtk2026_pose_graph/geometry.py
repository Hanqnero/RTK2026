...
Полилиния — последовательность точек ``((x, y), ...)`` от начала ребра
к концу. Направление обхода задаёт знак поперечного смещения и отсчёт
длины дуги.
"""
...
class PolylineProjection:
    """Проекция точки на полилинию: результат одного прохода по сегментам.

    Функции ниже — обёртки, каждая отдаёт одно поле. Когда нужно несколько,
    дешевле взять их отсюда, чем вызывать обёртки по очереди.
    """
...
def interpolate_along_polyline(
    ...
    Значения за пределами полилинии зажимаются к её концам, поэтому
    результат всегда лежит на пути.
