#!/usr/bin/env python3
"""
Autonomous frontier exploration node for ROS2.
Subscribes to /map, gets robot pose from TF (map -> base_footprint),
finds frontiers and sends goals to Nav2 NavigateToPose.
"""
...
class ExplorerNode(Node):
    def __init__(self):
        ...
        # use_sim_time is set by launch; do not declare here (ParameterAlreadyDeclaredException)
        ...
        self._blacklisted_goals = []  # list[(x,y)] to avoid repeating stuck goals
        self._nav_client = ActionClient(self, NavigateToPose, "navigate_to_pose")
        self._tf_buffer = Buffer()
        self._tf_listener = TransformListener(self._tf_buffer, self)
        self._idle = True
        self._goal_handle = None
        self._bootstrap_sent = False  # one initial goal when map has no frontiers
        self._last_goal_xy = None

        # /map is commonly published as transient_local; match it so we receive the latest map immediately.
        map_qos = QoSProfile(
            depth=1,
    ...
    def _robot_pose_in_map(self):
        """Return (x, y) or (x, y, yaw). Yaw in radians."""
        try:
            # Use latest available transform (Time(0)) to avoid sim-time extrapolation issues.
            when = Time()
            ...
            self.get_logger().debug("TF map->base_footprint: %s" % (e,))
            return None
    ...
    def _cluster_frontiers(self, frontiers):
        """Group adjacent frontier cells; keep only clusters >= min_frontier_size."""
        if not frontiers:
            return []
        fset = set(frontiers)
        clusters = []
    ...
    def _choose_frontier(self, clusters, robot_row, robot_col):
        ...
        for cluster in clusters:
            ...
            # Prefer farther frontiers to push into new space; otherwise choose nearest.
    ...
    def _explore_once(self):
        ...
        if pose is None:
            self.get_logger().info("Waiting for TF map->base_footprint (slam_toolbox and odom must be running)")
            return
        ...
        if not frontiers:
            # Bootstrap: if map is still empty (no free cells), drive 1.5 m forward so SLAM gets data
        ...
        # Filter out goals too close to robot (often causes spinning/recovery loops).
        ...
        if d_goal < self._min_goal_distance_m:
            ...
            return
        # Filter out goals near blacklisted points (previous failures/stuck).
