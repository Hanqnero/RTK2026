#!/usr/bin/env python3
"""Содержательная диагностика лидара, IMU и одометрии привода.
...
а также соответствие направления измеренного движения команде ``/cmd_vel``.
Проверка не зависит от устройства ходовой части.
"""
...
class DriveTask(DiagnosticTask):
    """Проверить одометрию и направление отклика привода."""
    ...
    def run(self, status):
        ...
        status.add("Command received", str(data["command_received"]))
        ...
        problems: list[tuple[int, str]] = []
        if not data["odom_received"]:
            problems.append((DiagnosticStatus.STALE, "Wheel odometry не получена"))
        elif not data["finite"]:
            problems.append((DiagnosticStatus.ERROR, "Wheel odometry содержит NaN/Inf"))
        ...
        if data["command_received"]:
            ...
            status.add("Command age (s)", f"{command_age:.3f}")
            if (
        ...
        if problems:
            ...
            status.summary(DiagnosticStatus.OK, "Одометрия привода корректна")
        return status
...
class SensorMonitor(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(
            TwistStamped,
            self.config["drive"]["command_topic"],
            self._on_command,
            10,
        )
    ...
    def _on_odometry(self, message: Odometry) -> None:
        with self._lock:
            ...
            self._odom_finite = _finite(
                [self._odom_linear, self._odom_angular]
            )
    ...
    def _record_direction_check(self) -> None:
        ...
        checked = [
            math.copysign(1.0, command_value)
            == math.copysign(1.0, odometry_value)
            for command_value, odometry_value in (
                (self._command_linear, self._odom_linear),
                (self._command_angular, self._odom_angular),
            )
            if (
                abs(command_value) > threshold
                and abs(odometry_value) > threshold
            )
        ]
        if checked:
