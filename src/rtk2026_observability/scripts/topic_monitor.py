...
class TopicConfig:
    ...
    :param graph_only: Проверять ROS graph без десериализации сообщений.
    :param minimum_publishers: Минимальное число publishers в graph-only режиме.
    :param minimum_subscribers: Минимальное число subscribers в graph-only режиме.
    """
...
class TopicHealthTask(DiagnosticTask):
    ...
    def run(self, status):
        ...
        if self._config.graph_only:
            return self._run_graph_check(status)
    ...
    def _run_graph_check(self, status):
        """Проверить endpoints topic без получения тяжёлых сообщений."""

        publishers = self._node.get_publishers_info_by_topic(
            self._config.topic
        )
        subscribers = self._node.get_subscriptions_info_by_topic(
            self._config.topic
        )
        published_types = sorted(
            {endpoint.topic_type for endpoint in publishers}
        )

        status.add("Topic", self._config.topic)
        status.add("Type", self._config.type_name)
        status.add("Check mode", "ROS graph only")
        status.add("Publishers", str(len(publishers)))
        status.add("Subscribers", str(len(subscribers)))
        status.add(
            "Published types",
            ", ".join(published_types) or "none",
        )

        problems: list[tuple[int, str]] = []
        if len(publishers) < self._config.minimum_publishers:
            level = (
                DiagnosticStatus.ERROR
                if self._config.required
                else DiagnosticStatus.OK
            )
            problems.append((level, "Недостаточно publishers"))
        if len(subscribers) < self._config.minimum_subscribers:
            problems.append(
                (DiagnosticStatus.ERROR, "Недостаточно subscribers")
            )
        if published_types and self._config.type_name not in published_types:
            problems.append(
                (DiagnosticStatus.ERROR, "Тип publisher не совпадает с ожидаемым")
            )

        if not problems:
            status.summary(DiagnosticStatus.OK, "ROS graph topic корректен")
            return status
...
class TopicMonitorNode(Node):
    ...
    def __init__(self) -> None:
        ...
        for key, values in topics.items():
            ...
            if not topic_config.graph_only:
            ...
            self.get_logger().info(
                ...
                f"graph_only={topic_config.graph_only}"
            )
    ...
    def _parse_topic_config(
        ...
        graph_only = bool(values.get("graph_only", False))
        minimum_publishers = int(values.get("minimum_publishers", 1))
        minimum_subscribers = int(values.get("minimum_subscribers", 0))
        ...
        if minimum_publishers < 0 or minimum_subscribers < 0:
            raise ValueError(
                f"topics.{key}: число endpoints не может быть отрицательным"
            )
