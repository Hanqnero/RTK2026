...
class GroundTruthTask(DiagnosticTask):
    ...
    def run(self, status):
        poses = self._monitor.pose_snapshot()
...
class LocalizationMonitor(Node):
    ...
    def _on_ground_truth(self, message: Odometry) -> None:
        pose = message.pose.pose
        with self._lock:
            self._poses["ground_truth"] = PoseSample(
                time.monotonic_ns(),
                _stamp_ns(message),
                float(pose.position.x),
                float(pose.position.y),
    ...
    def pose_snapshot(self) -> dict[str, PoseSample]:
        """Вернуть копию последних planar pose."""

        with self._lock:
            return {
                key: PoseSample(**vars(sample))
